...
resource "aws_iam_role" "ecs_task_execution_role" {
  name               = "ecs-task-execution-central"
...
##########################
# ECS Task Definitions and Services - Store1
##########################
resource "aws_ecs_task_definition" "store1_task" {
  family                   = "store1-task-central"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name      = "store1"
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.us-east-1.amazonaws.com/pos-store1:latest"
      essential = true
...
##########################
# ECS Task Definitions and Services - Store2
##########################
resource "aws_ecs_task_definition" "store2_task" {
  family                   = "store2-task-central"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "256"
  memory                   = "512"
  ...
  container_definitions = jsonencode([
    {
      name      = "store2"
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.us-east-1.amazonaws.com/pos-store2:latest"
      essential = true
      portMappings = [{ containerPort = 80, hostPort = 80 }]
      environment = [
...
##########################
# Lambda Replicator IAM Role
##########################
resource "aws_iam_role" "lambda_exec_role" {
  name = "pos-lambda-exec-role-central"
...
resource "aws_lambda_function" "replicator" {
  ...
  function_name = "dynamo-stream-replicator-central"
