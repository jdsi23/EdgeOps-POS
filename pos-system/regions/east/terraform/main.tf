...
resource "aws_ecs_cluster" "pos_cluster" {
  name = "pos-cluster-east"
}

resource "aws_ecs_task_definition" "store1_task" {
  family                   = "store1-task-east"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name      = "store1"
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.us-east-2.amazonaws.com/pos-store1:latest"
      essential = true
...
resource "aws_ecs_task_definition" "store2_task" {
  ...
  container_definitions = jsonencode([
    {
      name      = "store2"
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.us-east-2.amazonaws.com/pos-store2:latest"
      essential = true
      portMappings = [{ containerPort = 80, hostPort = 80 }]
      environment = [
...
resource "aws_iam_role" "lambda_exec_role" {
  name = "pos-lambda-exec-role-east"
...
resource "aws_lambda_function" "replicator" {
  ...
  function_name = "dynamo-stream-replicator-east"
