...
resource "aws_ecs_task_definition" "store1_task" {
  ...
  container_definitions = jsonencode([
    {
      name      = "store1"
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.us-west-1.amazonaws.com/pos-store1:latest"
      essential = true
...
resource "aws_ecs_task_definition" "store2_task" {
  ...
  container_definitions = jsonencode([
    {
      name      = "store2"
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.us-west-1.amazonaws.com/pos-store2:latest"
      essential = true
      portMappings = [{ containerPort = 80, hostPort = 80 }]
      environment = [
...
resource "aws_iam_role" "lambda_exec_role" {
  name = "pos-lambda-exec-role-south"
...
resource "aws_lambda_function" "replicator" {
  ...
  function_name = "dynamo-stream-replicator-south"
